      ...
      {userStore.user.wishlist && userStore.user.wishlist.length == 0 ? (
        <>
          <div className={"flex items-center justify-center min-h-[560px]"}>
            <div
              className={
                "h-fit w-fit px-16 py-6 rounded-xl flex flex-col items-center justify-center sm-w-fit md:mx-4 sm:mx-4"
              }
            >
              <h1 className={"font-semibold text-[#F5FEFD] text-4xl"}>
        ...
        <div className={"py-8 px-12 min-h-[80vh] bg-gray-900"}>
          <h1 className={"font-semibold text-3xl text-[#F5FEFD]"}>
            My Wishlist ({userStore.user?.wishlist?.length}{" "}
            {userStore.user?.wishlist?.length == 1 ? (
              <>
                <span>Property</span>
              </>
            ) : (
          ...
          <div className={"h-fit px-4 py-5 rounded-xl min-h-[768px]"}>
            {userStore.user.wishlist &&
              userStore.user?.wishlist.map((property) => {
                return (
                  <div className="rounded-[7px] my-5" key={property.propertyId}>
                    <div className="flex flex-col items-center border text-white rounded-lg shadow md:flex-row border-gray-700 bg-gray-800 hover:bg-gray-700 overflow-hidden">
                      ...
                      <div className="flex flex-col justify-around p-4 leading-normal ">
                        <div>
                          <Link to={`/properties/${property.propertyId}`}>
                            <h5 className="mb-4 text-3xl font-bold tracking-wide text-white">
                        ...
                        <div className="mb-2">
                          <p className="text-xl inline font-semibold tracking-tight text-white">
                            Address :{" "}
                          </p>
                          <p className="inline text-white tracking-wide">
                        ...
                        <div className="mb-2">
                          <p className="text-lg inline font-semibold tracking-tight text-white">
                            Price :{" "}
                          </p>
                          <p className="inline text-white tracking-wide">
                      ...
                      <button
                        className="ml-auto mt-auto cursor-pointer z-10 text-[#F5FEFD] hover:bg-red-500 bg-red-700 rounded-xl p-2 m-4"
                        onClick={(e) =>
                          handleRemoveFromCart(property.propertyId)
                        }
                      >
                        Remove
                      </button>
